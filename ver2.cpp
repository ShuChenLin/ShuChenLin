...
int dont_judge(int cnt, int live, int who) {
	...
	if (cnt == 2 && live == 1) return 5;
	if (cnt == 3 && live == 2) return 333;
	if (cnt == 3 && live == 1) return 33;
	if (cnt == 4 && live == 2) return 1250;
	if (cnt == 4 && live == 1) return 250;
	return 0;
}
