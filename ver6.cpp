...
int dont_judge(int cnt, int live, bool isblock, int who) {
	if (cnt >= 5 && !isblock) return 5000000;
	if (who == people) {
		if (isblock) {
			if (cnt >= 4 && live == 2) return 5000;
			if (cnt >= 5 && live == 1) return 5000;
			if (cnt == 4 && live == 1) return 2000;
			if (cnt == 3 && live == 2) return 50;
			if (cnt == 3 && live == 1) return 10;
		} else {
			if (cnt == 4 && live == 2) return 5000;
			if (cnt == 4 && live == 1) return 2000;
			if (cnt == 3 && live == 2) return 1000; //100
			if (cnt == 3 && live == 1) return 200;
			if (cnt == 2 && live == 2) return 10;
		}
	} else {
		if (isblock) {
			if (cnt >= 4 && live == 2) return 50000;
			if (cnt >= 4 && live == 1) return 5000;
			if (cnt == 3 && live == 2) return 2000;
			if (cnt == 3 && live == 1) return 50;
		} else {
			//if (cnt == 4 && live == 2) cout << "you are going to lose\n";
			if (cnt == 4 && live == 2) return 100000;
			if (cnt == 4 && live == 1) return 5000;
			if (cnt == 3 && live == 2) return 2501; // 1100
			if (cnt == 3 && live == 1) return 500; 
			if (cnt == 2 && live == 2) return 100;
...
node alpha_beta(int depth, int alpha, int beta, int who) {
	...
	if (!depth) {
		...
		nownode.val = player_score - 5 * computer_score;
...
bool live_three(int& x, int& y) {
	...
	for (auto it : next_move) {
		...
		if (s >= 100000) { 
			...
			if (s > tmpv) x = it.fi, y = it.se, tmpv = s;
			//x = it.fi, y = it.se;
		}
	}
	if (flag) return true;
